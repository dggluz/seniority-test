 ...
 */
export const readAndValidateJSONFile = <T> (path: string, contract: Contract<T>) =>
	readJSONFile(path)
...
/**
 * Moves a file from a path to another one, returning a Task.
 * @param originalPath File's original path
 * @param newPath File's new path
 */
